...
# controller to work and manipulate with Order model
class OrdersController < ApplicationController
  ...
  def check
    if !possible_orders
    ...
    elsif possible_cost > current_user.balance.to_f
      ...
      render json: {
        ...
        balance_after_transaction: current_user.balance.to_f - @cost
      }.to_json
    end
  end
  ...
  def possible_orders
    ...
    @specs['specs'].each do |conf|
      if  conf['os'].include?(params['os']) && conf['cpu'].include?(params['cpu'].to_i) &&
          conf['ram'].include?(params['ram'].to_i) && conf['hdd_type'].include?(params['hdd_type']) &&
